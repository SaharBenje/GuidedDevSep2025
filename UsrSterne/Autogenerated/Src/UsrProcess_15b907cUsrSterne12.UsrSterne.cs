namespace Terrasoft.Core.Process
{
	...
	#region Class: UsrProcess_15b907cUsrSterne12MethodsWrapper

	/// <exclude/>
	public class UsrProcess_15b907cUsrSterne12MethodsWrapper : ProcessModel
	{

		public UsrProcess_15b907cUsrSterne12MethodsWrapper(Process process)
			: base(process) {
		...
		private bool ScriptTask2Execute(ProcessExecutingContext context) {
			...
			if (andco != null && andco.user_docs != null && andco.user_docs.Count > 0) {
			    ...
			    foreach (var kvp in andco.user_docs) {
			        ...
			        string docid = GetLookupNameByCode("UsrTypeDJLookup", "UsrCode", doc.doc_id);
			        Guid? docidId = GetLookupIdByName("UsrTypeDJLookup", docid);
			        
			        if(docidId !=null)documentEntity.SetColumnValue("UsrTypeDocId", docidId.Value);
